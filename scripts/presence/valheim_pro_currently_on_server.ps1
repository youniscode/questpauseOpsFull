...
<#
  C:\QuestPauseOps\scripts\presence\valheim_pro_currently_on_server.ps1
  ...
  - Intended for both valheim_main and valheim_pro through -ServerKey
#>
...
function Get-RosterLines {
  ...
  $names = @($Roster.GetEnumerator() | Sort-Object Value | ForEach-Object { $_.Value })
  $lines = @()
  foreach ($n in $names) {
    $lines += "• $n"
  }
...
function Detect-QueryPort {
  ...
  foreach ($p in $Ports) {
    $info = Get-A2SInfo -TargetHost $TargetHost -Port $p -TimeoutMs $TimeoutMs
    if ($info) { return $p }
  }
  return $null
}
...
function Refresh-RosterTruth {
  ...
  if ($AutoDetectQueryPort -and $ResolvedQueryPortCandidates -and $ResolvedQueryPortCandidates.Count -gt 0) {
    $portToUse = Detect-QueryPort -TargetHost $ResolvedHost -Ports $ResolvedQueryPortCandidates -TimeoutMs $A2STimeoutMs
    if ($portToUse) {
  ...
  $players = Get-A2SPlayers -TargetHost $ResolvedHost -Port $portToUse -TimeoutMs $A2STimeoutMs
...
function Apply-LogLeave {
  ...
  $script:state.last_event_utc  = ($EventTime.ToUniversalTime()).ToString('o')
  $script:state.last_event_text = "LEAVE • $name"
  Save-State $script:state
  return $true
}
...
function Parse-ValheimLogLine {
  ...
  if ($line -match 'Peer\s*\((?<steam>\d{16,20})\)\s*disconnected') {
    ...
    $leaveSid = $Matches.steam.Trim()
  } elseif ($line -match 'SteamID=(?<steam>\d{16,20})') {
    $leaveSid = $Matches.steam.Trim()
  }
...
try {
  ...
  while ($true) {
    ...
    try {
      ...
      $beforeRosterHash = (($script:activeBySteam.GetEnumerator() | Sort-Object Name | ForEach-Object { "$($_.Name)=$($_.Value)" }) -join '|')

      [void](Refresh-RosterTruth)

      $afterRosterHash = (($script:activeBySteam.GetEnumerator() | Sort-Object Name | ForEach-Object { "$($_.Name)=$($_.Value)" }) -join '|')

      if ([bool]$script:state.last_a2s_ok -ne $a2sWasOk) { $changed = $true }
      if ($beforeRosterHash -ne $afterRosterHash) { $changed = $true }

...
param(
  ...
  # Pro / multi-instance identity override
...
<#
  C:\QuestPauseOps\scripts\status\valheim_live_pro_status.ps1
  ...
  - Config-driven world / webhook / max players / ports / exe path
  - Supports exact exe-path identity (best for multiple Valheim servers)
  - Falls back to port/cmdline/UDP ownership detection if exe path not configured
#>
...
if ($server.serverExePath) {
  $ExePathFallback = $server.serverExePath
} elseif ($server.exePath) {
  $ExePathFallback = $server.exePath
}
$ResolvedServerExePath = Get-FirstValue $ServerExePath $ExePathFallback
...
# For Valheim, require the game port for online detection.
# QueryPort is kept for info/debug only because WindowsGSM / Steam query behavior
# may not map cleanly to the same owning valheim_server.exe process.
$ExpectedUdpPorts = @($ResolvedGamePort)
$OptionalUdpPorts = @()
if ($ResolvedQueryPort -gt 0) { $OptionalUdpPorts += $ResolvedQueryPort }
...
Write-DebugLine "ServerExePath=$ResolvedServerExePath"
Write-DebugLine "ExpectedUdpPorts=$($ExpectedUdpPorts -join ',')"
Write-DebugLine "OptionalUdpPorts=$($OptionalUdpPorts -join ',')"
...
# Priority:
# 1) Exact exe path if configured
# 2) Command line -port match
# 3) UDP ownership on expected ports
# 4) Last-resort generic valheim_server process
# =========================
function Normalize-PathForCompare([string]$p) {
  if ([string]::IsNullOrWhiteSpace($p)) { return '' }
  try {
    return ([System.IO.Path]::GetFullPath($p)).Trim().ToLowerInvariant()
  } catch {
...
function Get-ValheimProcessByExactPath {
  if ([string]::IsNullOrWhiteSpace($ResolvedServerExePath)) { return $null }

  try {
    $target = Normalize-PathForCompare $ResolvedServerExePath
    Write-DebugLine "Exact-path identity enabled. Target='$target'"

    $procs = Get-CimInstance Win32_Process -Filter "Name='valheim_server.exe'" -ErrorAction SilentlyContinue
    if ($procs) {
      foreach ($p in $procs) {
        $exe = Normalize-PathForCompare $p.ExecutablePath
        if ($exe -and $exe -eq $target) {
          Write-DebugLine "MATCH by exact exe path (CIM): PID $($p.ProcessId) | $($p.ExecutablePath)"
          return $p
        }
        if ($p.ExecutablePath) {
          Write-DebugLine "Exact-path miss (CIM): PID $($p.ProcessId) | $($p.ExecutablePath)"
        } else {
          Write-DebugLine "Exact-path miss (CIM): PID $($p.ProcessId) | ExecutablePath unavailable"
        }
      }
    }

    $gps = Get-Process -Name 'valheim_server' -ErrorAction SilentlyContinue
    foreach ($gp in $gps) {
      try {
        $wmi = Get-WmiObject Win32_Process -Filter "ProcessId=$($gp.Id)" -ErrorAction SilentlyContinue
        $exe = Normalize-PathForCompare $wmi.ExecutablePath
        if ($exe -and $exe -eq $target) {
          Write-DebugLine "MATCH by exact exe path (WMI fallback): PID $($gp.Id) | $($wmi.ExecutablePath)"
        ...
        Write-DebugLine "Exact-path WMI fallback skip PID $($gp.Id): $($_.Exception.Message)"
      }
    }

    return $null
  } catch {
    Write-DebugLine "Get-ValheimProcessByExactPath ERROR: $($_.Exception.Message)"
    return $null
  }
}
  ...
  try {
    ...
    foreach ($p in $procs) {
      ...
      if (CommandLineHasPort -cmd $cmd -port $targetPort) {
        Write-DebugLine "MATCH by cmdline port: PID $($p.ProcessId) | $cmd"
        return $p
      }
    }

    foreach ($p in $procs) {
      if ($p.CommandLine) { Write-DebugLine "VALHEIM cmdline (no port match): PID $($p.ProcessId) | $($p.CommandLine)" }
...
function Get-ValheimProcessByUdpOwnership {
  ...
  foreach ($need in $ExpectedUdpPorts) {
    if ($ports -notcontains $need) { $allRequiredFound = $false; break }
  }
  if (-not $allRequiredFound) { continue }

  $gp = Get-Process -Id $ownerPid -ErrorAction SilentlyContinue
  if ($gp -and ($gp.ProcessName -like '*valheim*')) {
    Write-DebugLine "MATCH by UDP ownership: PID $ownerPid | ports=$($ports -join ',')"
    return [pscustomobject]@{
      ProcessId   = $ownerPid
      CommandLine = $null
    }
...
function Test-ProcessOwnsExpectedPorts {
  param([int]$ProcessId)

  try {
    if (-not (Has-Command "Get-NetUDPEndpoint")) { return $true }

    $allPortsToInspect = @($ExpectedUdpPorts + $OptionalUdpPorts)

    $ports = Get-NetUDPEndpoint -ErrorAction SilentlyContinue |
      Where-Object { $_.OwningProcess -eq $ProcessId -and ($allPortsToInspect -contains $_.LocalPort) } |
      Select-Object -ExpandProperty LocalPort

    $found = @($ports | Sort-Object -Unique)

    foreach ($need in $ExpectedUdpPorts) {
      if ($found -notcontains $need) {
        Write-DebugLine "PID $ProcessId missing REQUIRED UDP port $need. Found=[$($found -join ',')]"
        return $false
      }
    }

    if ($OptionalUdpPorts.Count -gt 0) {
      $missingOptional = @($OptionalUdpPorts | Where-Object { $found -notcontains $_ })
      if ($missingOptional.Count -gt 0) {
        Write-DebugLine "PID $ProcessId missing OPTIONAL UDP ports: $($missingOptional -join ','). Found=[$($found -join ',')]"
      }
    }

    Write-DebugLine "PID $ProcessId owns required UDP ports. Found=[$($found -join ',')]"
    return $true
  } catch {
    Write-DebugLine "Test-ProcessOwnsExpectedPorts ERROR for PID $ProcessId : $($_.Exception.Message)"
    return $false
  }
}

function Get-ValheimIdentityProcess {
  $pExact = Get-ValheimProcessByExactPath
  if ($pExact) { return $pExact }

  $pPort = Get-ValheimProcessByPort
  if ($pPort) { return $pPort }

  $pUdp = Get-ValheimProcessByUdpOwnership
  if ($pUdp) { return $pUdp }

  try {
    $gps = Get-Process -Name 'valheim_server' -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($gps) {
      Write-DebugLine "FALLBACK: generic valheim_server process exists but identity checks failed. PID $($gps.Id)"
      return [pscustomobject]@{
        ProcessId   = $gps.Id
        CommandLine = $null
      }
...
function Test-ValheimOnline {
  ...
  return (Test-ProcessOwnsExpectedPorts -ProcessId ([int]$p.ProcessId))
}
...
function Get-ValheimHealthLine {
  ...
  try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($ResolvedServerExePath)) {
      return "Online | PID $pid | RAM ${ramGB} GB | exact exe path + UDP verified"
    }
...
function Build-Payload {
  ...
  return [ordered]@{
    ...
    embeds = @(
      [ordered]@{
        title       = "VALHEIM Pro Server Status"
        description = $desc
        color       = $color
        fields      = @(
...
if ($RunOnce) {
  ...
  $maintenanceNow = $manualMaintenance
  ...
  $mode = if ($onlineNow -and -not $maintenanceNow) { 'online' } elseif ($maintenanceNow) { 'maintenance' } else { 'offline' }
...
Write-Host "ExpectedUdpPorts=$($ExpectedUdpPorts -join ',')"
Write-Host "ServerExePath=$ResolvedServerExePath"
...
while ($true) {
  try {
    ...
    $mode = if ($targetOnline -and -not $effectiveMaintenance) { 'online' } elseif ($effectiveMaintenance) { 'maintenance' } else { 'offline' }

...
<#  
  C:\QuestPauseOps\scripts\status\pz_live_status.ps1
  QUESTPAUSEOPS — Project Zomboid Live Status
...
$StateFile  = Join-Path $StateDir 'pz_status_state.json'
$DebugLog   = Join-Path $LogsDir  ("{0}_pz_live_status.log" -f $ServerKey)
$LastIdFile = Join-Path $StateDir 'pz_last_message_id.txt'
...
# Optional guard: product/type
if ($server.product -and $server.product -ne 'projectzomboid' -and $server.product -ne 'pz') {
...
} elseif ($server.udpPort) {
  $ResolvedQueryPort = [int]$server.udpPort
} else {
  $ResolvedQueryPort = 0
}
...
if ($ResolvedGamePort -le 0 -or $ResolvedQueryPort -le 0) {
  throw "GamePort/QueryPort missing for $ServerKey. Resolved GamePort=$ResolvedGamePort QueryPort=$ResolvedQueryPort"
}

$ExpectedUdpPorts = @($ResolvedGamePort, $ResolvedQueryPort)
...
# =========================
$OnlineFlavors = @(
  "Boot cycle complete systems green. Deploy on your signal.",
  "Uplink stable. Survivors can connect.",
  "Knox County reachable. Hold the line.",
  "Signal locked. World persists.",
  "Green across the board. Proceed."
)

$OfflineFlavors = @(
  "Uplink offline — no response from Knox County. Stand by.",
  "Radio silence. Server not responding.",
  "Signal lost. Waiting for uplink.",
  "No heartbeat detected. Stand by.",
  "Connection faded. Try again soon."
)

$MaintenanceFlavors = @(
  "Maintenance window active. Patching / cleanup underway.",
  "Controlled lockdown. Tuning the server.",
  "Testing changes. Reopening the gates shortly.",
  "Downtime on purpose. Back soon.",
  "Reinforcing the base. Hold tight."
)
...
function Get-PZProcessHealthLine {
  param([bool]$online)

  if (-not $online) { return "Pending uplink" }

  try {
    if (Has-Command "Get-NetUDPEndpoint") {
      $eps = @(Get-NetUDPEndpoint -ErrorAction SilentlyContinue | Where-Object { $ExpectedUdpPorts -contains $_.LocalPort })
      if ($eps.Count -gt 0) {
        $pid = ($eps | Select-Object -First 1).OwningProcess
        $p = Get-Process -Id $pid -ErrorAction Stop
        $ramGB = [math]::Round($p.WorkingSet64 / 1GB, 1)
        return "PID $pid | RAM ${ramGB} GB"
      }
    }

    $p = Get-Process -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -match '^ProjectZomboid|^ProjectZomboidServer|^java$' } |
...
# ONLINE CHECK
# =========================
function Test-PZOnline {
  try {
    if (Has-Command "Get-NetUDPEndpoint") {
      $eps = @(Get-NetUDPEndpoint -ErrorAction SilentlyContinue | Where-Object { $ExpectedUdpPorts -contains $_.LocalPort })
      if (-not $eps -or $eps.Count -eq 0) { return $false }

      $groups = $eps | Group-Object OwningProcess
      foreach ($g in $groups) {
        $ownedPorts = @($g.Group | Select-Object -ExpandProperty LocalPort | Sort-Object -Unique)
        $ok = $true
        foreach ($need in $ExpectedUdpPorts) {
          if ($ownedPorts -notcontains $need) { $ok = $false; break }
        }
        if ($ok) { return $true }
      }
      ...
      foreach ($need in $ExpectedUdpPorts) {
        if ($present -notcontains $need) { return $false }
      }
      return $true
    }

    $p = Get-Process -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -match '^ProjectZomboidServer$|^ProjectZomboid64$|^ProjectZomboid32$|^ProjectZomboid$|^java$' } |
      Select-Object -First 1

    return [bool]$p
  } catch {
    Write-DebugLine "Test-PZOnline error: $($_.Exception.Message)"
    return $false
  }
}
...
Write-Host "QuestPauseOps PZ watcher started for $ServerKey"
